...
describe( 'getBlockVersions', () => {
	test( "should keep the same versions if the blocks didn't change", () => {
		...
		expect(
			getBlockVersions( currentVersions, oldBlocks, newBlocks )
		).toEqual( currentVersions );
	} );
	...
	test( 'should increment block version on change', () => {
		...
		expect(
			getBlockVersions( currentVersions, oldBlocks, newBlocks )[ '1' ]
				.version
		).toEqual( 2 );
	} );
	...
	test( 'should increment block version on another change', () => {
		const currentVersions = {
			'1': { version: 1, nonce: '1' },
		};
		const oldBlocks = [
...
describe( 'getPositionVersions', () => {
	test( 'should keep the same version if no change', () => {
		...
		const oldBlocks = [];
		const newBlocks = oldBlocks;
		const result = getPositionVersions(
			currentVersions,
			oldBlocks,
			newBlocks
		);
